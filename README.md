...
- `ui_find(role, name, app="")`: elements matching a role and/or name, each with an opaque `ref`, `bounds`, decoded `states` and the title of its owning `window` (what tells equal matches in different windows apart). `app` scopes the search the same way as `ui_tree`. An empty match is a successful read, not an error.
