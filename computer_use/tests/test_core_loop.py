...
class TestTaskCompletion:
    def test_stops_when_task_marked_complete(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "do stuff", max_steps=10)
    ...
    def test_runs_up_to_max_steps(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "do stuff", max_steps=3)
...
class TestActionExecution:
    def test_executes_decided_action(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "type hello", max_steps=1)
    ...
    def test_records_action_failure(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "click it", max_steps=1)
...
class TestConsecutiveFailures:
    def test_aborts_after_max_consecutive_action_failures(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "task", max_steps=10)
...
class TestVerification:
    def test_verification_failure_marks_step_unsuccessful(self, _sleep):
        ...
        results = run_core_loop(
            capture, executor, provider, "task", max_steps=1, verify=True
        )
    ...
    def test_skips_verification_when_disabled(self, _sleep):
        ...
        results = run_core_loop(
            capture, executor, provider, "task", max_steps=1, verify=False
        )
...
class TestHistory:
    def test_builds_history_across_steps(self, _sleep):
        ...
        results = run_core_loop(capture, executor, provider, "task", max_steps=5)
