...
> `feature/0.7.8-broker-upgrade-handoff`; product head will be sealed before
> the first live cell. **vadgr-computer-use 0.7.8 evidence:** the private
> evidence pull request will be linked before the first live cell.
...
| S: WSL released upgrades | not run: S1-S3 require the candidate build | Not-Needed: no WSL-to-Windows path | Not-Needed: WSL origin is required | Not-Needed: no WSL-to-Windows path |
| R: candidate browser operation | not run: WSL half of R1 requires the candidate build | Not-Needed: no POSIX broker lifecycle change | not run: native Windows half of R1 requires the candidate build | Not-Needed: no POSIX broker lifecycle change |
