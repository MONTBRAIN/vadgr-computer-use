...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| B01 | One profile; two independent MCP clients | Attach both clients and list profiles, windows, and tabs | Both remain connected through one broker and see one complete registry | Both JSON streams and broker identity | pass: two independent clients retained one complete registry; unaffected observation retained | not run: audit repair candidate requires rerun; prior observation retained in history | pass: two independent installed clients retained one complete registry and broker identity | not run: audit repair candidate requires rerun; prior observation retained in history |
...
| B05 | One unowned tab in a shared user window | Client one claims the tab; client two reads it and attempts a window action | Client one can use tab-scoped DOM ops; client two receives `target_owned_by_another_client`; neither client changes window focus | Both streams and page record; release the tab | pass: foreign stale read and parent-window action failed before dispatch; fixture and focus unchanged | not run: audit repair candidate requires rerun; prior observation retained in history | pass: foreign tab read and parent-window action failed before dispatch; fixture and focus stayed unchanged | not run: audit repair candidate requires rerun; prior observation retained in history |
| B06 | Unowned tab and window targets | Race two clients for the tab, then race a window claim against a foreign child lease | One atomic winner exists; every loser gets the named conflict before dispatch | Both streams and lease revision record; release winner | pass: two independent CLI drivers overlapped both claim races; one winner and one named conflict each | not run: audit repair candidate requires rerun; prior observation retained in history | pass: simultaneous paired claims produced one atomic winner and one named conflict in both races | not run: audit repair candidate requires rerun; prior observation retained in history |
| B07 | Owned window, child tabs, popup, and shared window | Open tabs with and without opener, a popup, and an unattributable shared-window child | Owned descendants inherit the correct lease; the unattributable shared child stays unowned | Registry snapshots; close created targets | pass: opener inheritance and unrelated unowned child observed; unaffected observation retained | not run: audit repair candidate requires rerun; prior observation retained in history | pass: opener descendants inherited leases and an unrelated shared child stayed unowned after trusted dispatch repair | not run: audit repair candidate requires rerun; prior observation retained in history |
| B08 | Client owns one tab and one multi-tab window | Release each, then dispatch with the old revision | Browser state stays open, leases clear, and stale revisions fail | Registry and page records; close created state | pass: releases kept resources open and stale revisions were refused | not run: audit repair candidate requires rerun; prior observation retained in history | pass: releases kept resources open and old revisions failed without mutation | not run: audit repair candidate requires rerun; prior observation retained in history |
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: repaired candidate restored the 8.032-second cut; 46.063-second cut orphaned leases before explicit claims | not run: audit repair candidate requires rerun; prior observation retained in history | pass: 8.031-second cut retained the lease; 46.031-second cut expired it for explicit reclaim | not run: audit repair candidate requires rerun; prior observation retained in history |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: both first stale requests returned target_lost before status; explicit reclaim preserved documents | not run: audit repair candidate requires rerun; prior observation retained in history | pass: replacement broker changed identity, first stale calls returned target_lost, and documents survived reclaim | not run: audit repair candidate requires rerun; prior observation retained in history |
...
**Current verdict: WSL and native Windows complete.** Each has 20 accepted
observations from its 20-cell round. The repaired candidate's current CI must
complete on the implementation PR. Native Linux and macOS remain as shown.
...
| B: shared broker and target ownership | pass: every required WSL observation accepted, including concurrent B02f | blocked: B01 and B05-B10 await audit repairs and candidate reruns | pass: all required ownership and cross-OS observations accepted, including concurrent B02f | blocked: B01 and B05-B10 await audit repairs and candidate reruns |
