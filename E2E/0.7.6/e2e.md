...
**Current close status: WSL and native Windows complete.**
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| B01 | One profile; two independent MCP clients | Attach both clients and list profiles, windows, and tabs | Both remain connected through one broker and see one complete registry | Both JSON streams and broker identity | pass: two independent clients retained one complete registry; unaffected observation retained | pass: two fresh installed clients shared one broker identity and complete registry at d794ad1 | pass: two independent installed clients retained one complete registry and broker identity | not run: audit repair candidate requires rerun; prior observation retained in history |
...
| B05 | One unowned tab in a shared user window | Client one claims the tab; client two reads it and attempts a window action | Client one can use tab-scoped DOM ops; client two receives `target_owned_by_another_client`; neither client changes window focus | Both streams and page record; release the tab | pass: foreign stale read and parent-window action failed before dispatch; fixture and focus unchanged | pass: foreign tab and parent-window actions failed before dispatch while fixture and focus stayed unchanged at d794ad1 | pass: foreign tab read and parent-window action failed before dispatch; fixture and focus stayed unchanged | not run: audit repair candidate requires rerun; prior observation retained in history |
| B06 | Unowned tab and window targets | Race two clients for the tab, then race a window claim against a foreign child lease | One atomic winner exists; every loser gets the named conflict before dispatch | Both streams and lease revision record; release winner | pass: two independent CLI drivers overlapped both claim races; one winner and one named conflict each | pass: bounded MCP barriers queued both claims before either result; each tab and window race had one winner and one named conflict | pass: simultaneous paired claims produced one atomic winner and one named conflict in both races | not run: audit repair candidate requires rerun; prior observation retained in history |
| B07 | Owned window, child tabs, popup, and shared window | Open tabs with and without opener, a popup, and an unattributable shared-window child | Owned descendants inherit the correct lease; the unattributable shared child stays unowned | Registry snapshots; close created targets | pass: opener inheritance and unrelated unowned child observed; unaffected observation retained | pass: opener descendants inherited exact leases while the unrelated shared child stayed unowned after the trusted-click repair | pass: opener descendants inherited leases and an unrelated shared child stayed unowned after trusted dispatch repair | not run: audit repair candidate requires rerun; prior observation retained in history |
| B08 | Client owns one tab and one multi-tab window | Release each, then dispatch with the old revision | Browser state stays open, leases clear, and stale revisions fail | Registry and page records; close created state | pass: releases kept resources open and stale revisions were refused | pass: released resources stayed open and old revisions were fenced without mutation at d794ad1 | pass: releases kept resources open and old revisions failed without mutation | not run: audit repair candidate requires rerun; prior observation retained in history |
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: repaired candidate restored the 8.032-second cut; 46.063-second cut orphaned leases before explicit claims | blocked: the 11.627-second reconnect retained its lease, but VM scheduling delayed the 46-second relay restoration to 64.606 seconds and the strict 55-second controller rejected that leg | pass: 8.031-second cut retained the lease; 46.031-second cut expired it for explicit reclaim | not run: audit repair candidate requires rerun; prior observation retained in history |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: both first stale requests returned target_lost before status; explicit reclaim preserved documents | pass: PID 122294/epoch 25fde4b9 changed to PID 145054/epoch 89d79d14; first stale request returned target_lost and explicit reclaim preserved the document | pass: replacement broker changed identity, first stale calls returned target_lost, and documents survived reclaim | not run: audit repair candidate requires rerun; prior observation retained in history |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | pass: fresh repaired wheel started publicly; version, segmenter, profile and checkout-absent imports verified | pass: fresh noneditable wheel beb3d064 reported 0.7.6, loaded Unicode segmentation and the fitted profile, and imported outside the checkout | pass: fresh noneditable 0.7.6 wheel loaded all resources with checkout-absent imports and public readiness | not run: audit repair candidate requires rerun; prior observation retained in history |
...
**Current verdict: WSL and native Windows complete; native Linux has one
environment-blocked timing cell.** WSL and Windows each have 20 accepted
observations from their 20-cell rounds. Linux accepted 14 of its 15 owed
observations; B09 is blocked by a VM scheduling delay recorded below. The
repaired candidate's current CI must complete on the implementation PR. macOS
remains as shown.
The tables in Parts A-E retain earlier observed results where unaffected. The
...
| B: shared broker and target ownership | pass: every required WSL observation accepted, including concurrent B02f | blocked: B01, B05-B08 and B10 passed; B09's long relay controller exceeded its VM timing bound | pass: all required ownership and cross-OS observations accepted, including concurrent B02f | blocked: B01 and B05-B10 await audit repairs and candidate reruns |
...
| E: packaged and clean delivery | pass: E01, E03 and E04 accepted | pass: E01 fresh noneditable candidate identity and public readiness accepted | pass: E01, E03 and E04 accepted on the repaired installed wheel | blocked: E01 requires candidate identity |
| F: final audit regressions | pass: F01-F05 and F08 accepted; F06-F07 are POSIX Not-Needed on WSL | pass: F01-F07 accepted with exact profile, ownership, publication and recovery oracles | pass: F01-F05 and F08 accepted; F06-F07 are POSIX Not-Needed on Windows | blocked: F01-F07 await repairs and execution |
| overall | pass: 20 of 20 required observations accepted; evidence publication and post-push cleanup remain | blocked: 14 of 15 owed observations accepted; B09 is environment-blocked by the retained 64.606-second relay restoration | pass: 20 of 20 required observations accepted; evidence published and scoped cleanup complete | blocked: candidate gate and named reruns remain owed |
