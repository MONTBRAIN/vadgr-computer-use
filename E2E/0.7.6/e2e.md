...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| D01 | Native event-recording field focused. On WSL, give the isolated Windows test window a unique title and use `harness/focus_window.ps1 -ExactTitle` to verify its exact non-minimized HWND is foreground. | Run fast `type_text` and named-profile human `type_text` | Fast mode stays compatible; human mode emits ordered varied input and exact final text | Native event record with text removed; clear field | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| D02 | D01 field focused | Run custom WPM plus IKI-CV, including zero variation | Planned and achieved metadata are valid; zero variation is constant control | Timing metadata only; clear field | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| D03 | D01 field focused | Try invalid option combinations and a deadline shorter than the plan | Each fails before input and leaves the field unchanged | Error results and independent field read-back | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| D04 | D01 field focused | Complete a plan longer than 60 seconds without a timeout, then exercise runtime deadline, cancellation and supported Unicode or fallback | Long input completes; deadline and cancellation report a truthful prefix; modifiers are released; exact text or named fallback is reported; no whole-string retry or submit occurs after interruption | Event record with text removed; release keys and clear field | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| D05 | Human pixel typing active in one client | Start a second pixel keyboard action | The product makes no parallel-safety claim; evidence records machine-global serialization or conflict behavior exactly | Both streams; stop both operations and release keys | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |

## Part E: packaged and clean delivery

| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | not run: package changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| E02 | Matching store-equivalent extension and installed wheel. WSL uses D01's exact foreground-window setup before the pixel action. | Run one owned-window browser read, one human browser type longer than 60 seconds, and one human pixel type longer than 60 seconds | The installed package and matching extension execute long typing without an implicit total deadline | MCP JSON with input text removed; close test state | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
...
| C: actionability and browser typing | not run: affected typing cells require rerun | not run: remote host | not run: remote host | not run: remote host |
| D: pixel typing | not run: typing implementation changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| E: packaged and clean delivery | not run: package changed after prior pass | not run: remote host | not run: remote host | not run: remote host |
| overall | not run: affected typing and package cells require rerun | not run: remote host | not run: remote host | not run: remote host |
