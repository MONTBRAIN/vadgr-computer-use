...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| B01 | One profile; two independent MCP clients | Attach both clients and list profiles, windows, and tabs | Both remain connected through one broker and see one complete registry | Both JSON streams and broker identity | pass: independent WSL and Windows clients shared one broker and caller-specific registry at 747b6fb | not run: target-label repair rerun owed; prior functional observations retained | pass: two independent clients shared one complete broker identity and registry at 86fe6f1 | pass: two independent clients shared one broker and registry with caller-specific target labels |
...
| B03 | Two clients in one profile | Each creates an owned window and two tabs, then interleaves reads and mutations | Responses return only to the caller; each sees foreign targets as owned by another client | Both streams and page event records; close test windows | pass: repaired-artifact dual clients keep caller-specific mutation targets at 5d4f873 | not run: mutation-response metadata repair rerun owed; prior observation retained | pass: separate owned windows and tabs retained caller-specific targets; foreign access rejected; close annotations verified at 86fe6f1 | not run: mutation-response metadata repair rerun owed; prior observation retained |
| B04 | B03 windows remain | Select a different current tab per client and interleave operations | Every request reaches its exact profile, window, and tab without cross-routing | Target ids and structured page records | pass: four tabs retain distinct counters and exact caller targets at 5d4f873 | not run: mutation-response metadata repair rerun owed; prior observation retained | pass: four document counters stayed isolated through interleaved mutations and target switches at 86fe6f1 | not run: mutation-response metadata repair rerun owed; prior observation retained |
...
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: timely reconnect retained the lease; expiry orphaned it and required explicit reclaim at 747b6fb | not run: rerun the setup-diagnosis repair; prior observation retained | pass: actual reconnect 10.813s after an 8s cut retained target, revision 1 and counter 101; after a 46s cut the reconnect at 48.860s left the lease orphaned, the first read returned target_owned_by_another_client, and the explicit reclaim returned created:false on the same window with counter 101 and its original timeOrigin at revision 4, while the second client held counter 202 at revision 2 throughout | pass: timely reconnect retained identity and leases; expired leases required explicit reclaim |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: selected stale targets fail; explicit reclaim preserves both documents at 5d4f873 | not run: mutation-response metadata repair rerun owed; prior observation retained | pass: stale calls returned target_lost; orphaned targets explicitly reclaimed with original documents and counters at 86fe6f1 | not run: mutation-response metadata repair rerun owed; prior observation retained |
...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | not run: rerun normal replacement startup after the spaced-path launcher repair; prior 5d4f873 pass retained | Not-Needed: no cross-OS seam | pass: public operations started one replacement broker after the exact test PID stopped; both clients converged and reclaimed original documents at 86fe6f1 | Not-Needed: no cross-OS seam |
...
|---|---|---|---|---|
| A: recovery and setup diagnosis | incomplete: A03 redirected-local-appdata rerun owed; prior observations retained | incomplete: rerun A01-A04 after the setup-diagnosis repair | incomplete: A01, A03 and A04 passed; A02 was approved and its suspend ran, but its single gated read needs one driver session spanning the suspend and the owner declined a second one | pass: A01-A04 observed; A02 retained the original document after verified sleep and full wake |
| B: shared broker and target ownership | incomplete: B15 requires the spaced-path launcher repair | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: affected ownership and restart cells passed, including B09 under a second driver after the first hit its account usage limit mid cell | incomplete: mutation-response repair requires fresh affected-cell observations |
...
| E: packaged and clean delivery | incomplete: fresh launcher-artifact E01 and new E03 are owed | incomplete: mutation-response repair requires fresh affected-cell observations | pass: installed 86fe6f1 wheel identity, readiness and spaced-path normal broker startup verified | incomplete: mutation-response repair requires fresh affected-cell observations |
| overall | incomplete: Windows launcher and rebuilt bundle require A03, E01, E03 and B15; prior observations retained | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: every rerun cell passed, B09 included; A02 alone remains, its suspend observed but its gated read not validly exercised | incomplete: mutation-response repair requires fresh affected-cell observations |
