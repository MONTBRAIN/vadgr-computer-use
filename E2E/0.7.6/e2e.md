...
| B06 | Unowned tab and window targets | Race two clients for the tab, then race a window claim against a foreign child lease | One atomic winner exists; every loser gets the named conflict before dispatch | Both streams and lease revision record; release winner | pass: two independent CLI drivers overlapped both claim races; one winner and one named conflict each | pass: bounded MCP barriers queued both claims before either result; each tab and window race had one winner and one named conflict | pass: simultaneous paired claims produced one atomic winner and one named conflict in both races | blocked: current claim-race round not run before provider verification refusal |
| B07 | Owned window, child tabs, popup, and shared window | Open tabs with and without opener, a popup, and an unattributable shared-window child | Owned descendants inherit the correct lease; the unattributable shared child stays unowned | Registry snapshots; close created targets | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 rerun owed; c2189e9 partial observations lack unattributable-child proof |
| B08 | Client owns one tab and one multi-tab window | Release each, then dispatch with the old revision | Browser state stays open, leases clear, and stale revisions fail | Registry and page records; close created state | pass: releases kept resources open and stale revisions were refused | pass: released resources stayed open and old revisions were fenced without mutation at d794ad1 | pass: releases kept resources open and old revisions failed without mutation | blocked: partial releases observed at c2189e9; exact stale-revision oracle remains unverified |
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: repaired candidate restored the 8.032-second cut; 46.063-second cut orphaned leases before explicit claims | blocked: the 11.627-second reconnect retained its lease, but VM scheduling delayed the 46-second relay restoration to 64.606 seconds and the strict 55-second controller rejected that leg | pass: 8.031-second cut retained the lease; 46.031-second cut expired it for explicit reclaim | blocked: current reconnect round not run before provider verification refusal; prior pass retained |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: both first stale requests returned target_lost before status; explicit reclaim preserved documents | pass: PID 122294/epoch 25fde4b9 changed to PID 145054/epoch 89d79d14; first stale request returned target_lost and explicit reclaim preserved the document | pass: replacement broker changed identity, first stale calls returned target_lost, and documents survived reclaim | blocked: current broker-replacement round not run before provider verification refusal; prior pass retained |
...
| C06 | C05 state plus state-bearing pointer, reveal, focus, file-input, dialog and semantic controls | Run trusted `click`, `hover`, `focus`, `blur`, `upload`, `snapshot`, `accessibility_tree`, `eval`, navigation, reload, back and forward; arm the dialog on the leased tab while a decoy target also emits one | Every operation reaches the leased tab, carries an independent state/read-back oracle, ignores the decoy dialog and preserves false/false/true; cookie operations are excluded because they are profile/origin scoped | Before/after target state, state-bearing results, temporary upload hash and decoy hash; remove the temporary file and close only the owned window | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | fail: inactive trusted clicks twice returned success with counter zero at c2189e9; d7b6aa9 repair remains unverified after provider refusal |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: new extension artifact verification owed; c2189e9 installed identity and ASCII/profile observations passed |
...
**Current verdict: incomplete on all four hosts.** E01, B07, C06 and F05 reopened
with `d7b6aa9`. The preceding WSL and Windows completed rounds remain historical.
Linux also retains its B09 scheduling blocker. macOS is a partial live pass,
not unexecuted: B01/B05/F01/F02/F06 passed at c2189e9. C06 failed twice and its
repair remains unverified. A provider safety refusal stopped further live
verification. Two native-host interpreter shutdown fatals also remain unresolved.
Unaffected passing observations retain their original identities and oracles.
Further live verification needs an owner decision. Do not bypass the refusal
through prompt changes or model substitution.

| part | WSL | Linux | Windows | macOS |
|---|---|---|---|---|
| A: recovery and setup diagnosis | pass: unaffected observations retained | pass: unaffected observations retained | pass: unaffected observations retained | pass: unaffected observations retained |
| B: shared broker and target ownership | blocked: B07 rerun owed; other accepted results retained | blocked: B07 rerun owed; B09 scheduling blocker retained | blocked: B07 rerun owed; other accepted results retained | blocked: B01/B05 pass; B06-B10 incomplete or owed |
| C: actionability and browser typing | blocked: C06 rerun owed; unaffected cells retained | blocked: C06 rerun owed; unaffected cells retained | blocked: C06 rerun owed; unaffected cells retained | fail: repeated inactive trusted-click mismatch; repair unverified |
...
| E: packaged and clean delivery | blocked: E01 fresh artifact verification owed | blocked: E01 fresh artifact verification owed | blocked: E01 fresh artifact verification owed | blocked: prior c2189e9 E01 passed; new artifact verification owed |
| F: final audit regressions | blocked: F05 rerun owed; other accepted results retained | blocked: F05 rerun owed; other accepted results retained | blocked: F05 rerun owed; other accepted results retained | blocked: F01/F02/F06 pass; F03/F04/F05/F07 incomplete; shutdown finding unresolved |
| overall | blocked: E01/B07/C06/F05 reopened | blocked: E01/B07/C06/F05 reopened; B09 environment blocker retained | blocked: E01/B07/C06/F05 reopened | blocked: partial pass, C06 failure with unverified repair, provider refusal and unresolved shutdown finding |
...
the independent click event. Working active trusted and inactive DOM controls
rule out a missing button or broken counter. Repair d7b6aa9 requires a new
installed-artifact live repeat; the old C06 result is not sufficient.

F07 observed publication refusal, protected stale state and no remaining failed
publisher or temporary file. Explicit disarm succeeded. Its restoration driver
received a provider safety refusal before any public product call. MCP bootstrap
had already started another broker, but no public readiness or original-document
read-back followed. No prompt retry or model substitution bypassed the refusal.

The closed browser stderr also contains two native-host interpreter shutdown
fatals naming `_enter_buffered_busy` and the standard-input buffered lock.
A source audit finds a daemon input thread reading that stream while the main
native-host path can return after broker EOF without joining it. This is a
source-supported cause hypothesis, not conclusive attribution of either fatal:
the fatal lines carry no PID or timestamp. Chrome exit zero does not establish
native-host exit success. The finding remains unresolved and requires its own
regression and verification; it is not hidden by the provider blocker.
