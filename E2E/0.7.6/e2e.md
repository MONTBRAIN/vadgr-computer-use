...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | not run: verify replacement startup after the heartbeat-EOF repair; prior observations retained | Not-Needed: no cross-OS seam | pass: both direct clients converged on one new Windows broker PID and epoch; rediscovered leases were orphaned before explicit reclaim at 8ffe269 | Not-Needed: no cross-OS seam |
