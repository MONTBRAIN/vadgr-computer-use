...
| B03 | Two clients in one profile | Each creates an owned window and two tabs, then interleaves reads and mutations | Responses return only to the caller; each sees foreign targets as owned by another client | Both streams and page event records; close test windows | fail: interleaved reads passed, but closing the WSL window returned the other client's target metadata | not run: target-label repair rerun owed; prior functional observations retained | not run: target-label repair rerun owed; prior functional observations retained | pass: owned windows retained separate leases, target labels and exact DOM hashes |
| B04 | B03 windows remain | Select a different current tab per client and interleave operations | Every request reaches its exact profile, window, and tab without cross-routing | Target ids and structured page records | pass: four exact tab read-backs retained separate state and caller-specific targets at 747b6fb | not run: target-label repair rerun owed; prior functional observations retained | not run: target-label repair rerun owed; prior functional observations retained | pass: interleaved tab operations retained each caller's selected target and exact read-back |
...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | pass: WSL and native Windows converged on one new Windows PID and epoch at 747b6fb | Not-Needed: no cross-OS seam | not run: rerun the setup-diagnosis repair; prior observation retained | Not-Needed: no cross-OS seam |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | pass: fresh installed wheel at 747b6fb matched all 100 payload members and reported WSL readiness | not run: rerun the setup-diagnosis repair; prior observation retained | not run: rerun the setup-diagnosis repair; prior observation retained | pass: 381993f fresh noneditable wheel reports 0.7.6, no checkout import path and installed segmenter; public default-profile typing loads the fitted profile and matches its exact native hash |
...
| B: shared broker and target ownership | fail: B03 close response carries a foreign target; the other owed B reruns passed | incomplete: rerun B09 and B10 after the repair | incomplete: rerun B09, B10 and B15 after the repair | pass: all applicable ownership, race, reconnect and restart cells observed |
...
| E: packaged and clean delivery | pass: E01 fresh-wheel identity rerun passed; prior E02 observations retained | incomplete: E01 identity on the repaired package | incomplete: E01 identity on the repaired package | pass: repaired installed identity and default-profile load observed; both unaffected long-input paths retained |
| overall | fail: B03 response metadata repair is owed; A04 is blocked on registration-fault permission | incomplete: setup and recovery reruns remain | incomplete: setup and recovery reruns remain | pass: every applicable macOS cell has an observed passing result |
...
- The WSL rerun at `747b6fb` found a remaining response-label defect. After
  ...
  target. Three captures reproduced it. The owned windows and interleaved DOM
  read-backs remained isolated; no cross-client input was observed. B03 fails
  until 0.7.6 removes the shared target from mutation replies and verifies
  caller-specific metadata. This is not a completed host pass.
