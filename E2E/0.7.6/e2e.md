...
**Current close status: incomplete on all hosts after the trusted-click repair.**
The extension repair at `d7b6aa9` reopens E01, B07, C06 and F05 everywhere.
The following completed rounds are historical, not acceptance of that repair.
The prior observations remain recorded below. WSL and native Windows have each
verified all 20 required observations on the repaired candidate, including
E03, E04, B02f and F08.
...
### Latest extension repair and macOS audit
...
`d7b6aa99f9c76ed2390020053f03cb4ef3e88c9e` remains unverified live.
...
| B07 | Owned window, child tabs, popup, and shared window | Open tabs with and without opener, a popup, and an unattributable shared-window child | Owned descendants inherit the correct lease; the unattributable shared child stays unowned | Registry snapshots; close created targets | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | pass: same-window child and trusted popup inherited their lease while a DevTools-created shared child without an opener stayed unowned at bf92090 |
...
| C06 | C05 state plus state-bearing pointer, reveal, focus, file-input, dialog and semantic controls | Run trusted `click`, `hover`, `focus`, `blur`, `upload`, `snapshot`, `accessibility_tree`, `eval`, navigation, reload, back and forward; arm the dialog on the leased tab while a decoy target also emits one | Every operation reaches the leased tab, carries an independent state/read-back oracle, ignores the decoy dialog and preserves false/false/true; cookie operations are excluded because they are profile/origin scoped | Before/after target state, state-bearing results, temporary upload hash and decoy hash; remove the temporary file and close only the owned window | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | pass: active and inactive trusted clicks delivered isTrusted events; all state, upload, semantic, navigation and leased-dialog oracles passed with the decoy isolated at bf92090 |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | pass: fresh noneditable wheel f19d2fe, matching development extension fe80121c and installed entry point b75bffcc passed readiness, Unicode and fitted-profile checks outside the checkout at bf92090 |
...
| F05 | A owns an opener target; a real opener-linked popup in another window inherits A's window lease; A selects the popup tab; B is independent | A releases the popup window without changing its selection; both clients list twice; B claims the popup window and uses its existing tab; A attempts a stale popup write before any new selection; inspect opener afterward | Popup stays open and unowned after release and repeated lists; B receives a usable lease; A's stale write fails with target_owned_by_another_client without closing or mutating it; A retains only its opener | Both streams, opener relationship, window/tab leases and revisions, popup read-back and unchanged opener; release and close only test windows | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | not run: d7b6aa9 changes the extension click path; fresh artifact and affected live rerun owed; prior result retained | pass: repeated lists left the released popup unowned; B claimed it, A's stale write failed, and popup/opener read-backs remained exact at bf92090 |
...
**Current verdict: macOS complete; WSL, Linux, and Windows remain incomplete.**
E01, B07, C06 and F05 reopened with `d7b6aa9` on those three hosts. Their
preceding completed rounds remain historical, and Linux also retains its B09
scheduling blocker. The macOS audit completed every reopened or affected cell
...
| B: shared broker and target ownership | blocked: B07 rerun owed; other accepted results retained | blocked: B07 rerun owed; B09 scheduling blocker retained | blocked: B07 rerun owed; other accepted results retained | pass: B06-B10 reran at bf92090; unaffected observations retained |
| C: actionability and browser typing | blocked: C06 rerun owed; unaffected cells retained | blocked: C06 rerun owed; unaffected cells retained | blocked: C06 rerun owed; unaffected cells retained | pass: C06 reran at bf92090; unaffected observations retained |
...
| E: packaged and clean delivery | blocked: E01 fresh artifact verification owed | blocked: E01 fresh artifact verification owed | blocked: E01 fresh artifact verification owed | pass: fresh E01 installed-artifact verification at bf92090; unaffected observations retained |
| F: final audit regressions | blocked: F05 rerun owed; other accepted results retained | blocked: F05 rerun owed; other accepted results retained | blocked: F05 rerun owed; other accepted results retained | pass: F03-F05 and F07 reran at bf92090; unaffected observations retained |
| overall | blocked: E01/B07/C06/F05 reopened | blocked: E01/B07/C06/F05 reopened; B09 environment blocker retained | blocked: E01/B07/C06/F05 reopened | pass: all 32 applicable cells have accepted observations; 15 cells are Not-Needed with retained reasons |
