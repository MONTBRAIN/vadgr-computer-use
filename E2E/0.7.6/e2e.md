...
| B03 | Two clients in one profile | Each creates an owned window and two tabs, then interleaves reads and mutations | Responses return only to the caller; each sees foreign targets as owned by another client | Both streams and page event records; close test windows | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained |
| B04 | B03 windows remain | Select a different current tab per client and interleave operations | Every request reaches its exact profile, window, and tab without cross-routing | Target ids and structured page records | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained | not run: mutation-response metadata repair rerun owed; prior observation retained |
...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | not run: mutation-response metadata repair rerun owed; prior observation retained | Not-Needed: no cross-OS seam | not run: mutation-response metadata repair rerun owed; prior observation retained | Not-Needed: no cross-OS seam |
...
| E: packaged and clean delivery | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: mutation-response repair requires fresh affected-cell observations |
| overall | incomplete: mutation-response reruns and A04 registration-fault permission are owed | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: mutation-response repair requires fresh affected-cell observations | incomplete: mutation-response repair requires fresh affected-cell observations |
...
- Source repair `7d5c7bf` replaces shared mutation target annotations with the
  ...
  B03, B04, B10, applicable B15 and E01 need new observations on each host.
