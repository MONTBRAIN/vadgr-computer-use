...
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: actual reconnect in 16.313 seconds retained the lease; expiry at 52.969 seconds required explicit reclaim with unchanged document and peer at 9a564b9 | pass: repaired-wheel reconnect in 12.842 seconds retained the lease; expired ownership required explicit reclaim without document loss at 17fbe37 | pass: repaired-wheel reconnect in 8.141 seconds retained the lease; expiry at 46.156 seconds required explicit reclaim without document loss or peer disruption at 8ffe269 | pass: b207269 reconnect in 14.506 seconds retained the lease; expiry required explicit reclaim without document loss or peer disruption |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: both stale clients returned target_lost after broker replacement; explicit reclaim preserved four original documents at 9a564b9 | pass: both stale clients return target_lost after broker replacement; explicit orphan reclaim preserves all four original documents at 17fbe37 | pass: both stale clients returned target_lost after verified broker termination; explicit reclaim preserved all four original documents at 8ffe269 | pass: both stale clients returned target_lost after broker replacement; explicit reclaim preserved all four documents at b207269 |
...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | pass: both direct clients converged on one new Windows broker identity; original leases were orphaned before explicit reclaim at 9a564b9 | Not-Needed: no cross-OS seam | pass: both direct clients converged on one new Windows broker PID and epoch; rediscovered leases were orphaned before explicit reclaim at 8ffe269 | Not-Needed: no cross-OS seam |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | pass: fresh noneditable b2b1525 wheel starts outside checkout; public readiness, Unicode segmenter and profile verified | pass: fresh noneditable 17fbe37 wheel starts outside checkout; public readiness, Unicode segmentation and fitted profile loading succeed | pass: native Windows fresh noneditable b2b1525 wheel starts outside checkout; public readiness, Unicode segmenter and profile verified | pass: fresh noneditable b207269 wheel starts outside checkout; public readiness, Unicode segmentation and fitted profile loading succeed |
...
| E03 | Fresh installed wheel; spaced Windows deployment parent; no test broker running | Request public readiness, open the fixture and read it without manually launching the broker | Normal startup creates one verified broker and the public read succeeds | Path-shape booleans, wheel and bundle hashes, broker identity and MCP results; restore registrations and stop only test processes | pass: automatic startup from zero brokers under a spaced path and incompatible parent module path; exact public fixture read at b2b1525 | Not-Needed: native Linux does not execute the Windows launcher | pass: native Windows automatic startup from zero brokers under a spaced path and incompatible parent module path; exact public fixture read at b2b1525 | Not-Needed: native macOS does not execute the Windows launcher |
