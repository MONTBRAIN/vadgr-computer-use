...
**Current close status: WSL, Linux, Windows and macOS complete.**
The extension repair at `d7b6aa9` reopened E01, B07, C06 and F05 everywhere.
WSL reran all four at branch head `2845224`, whose latest runtime source is
`bf92090`; macOS had already completed them at `bf92090`. The preceding WSL and
native Windows rounds remain historical where they predate the extension
repair. Their other accepted observations, including E03, E04, B02f and F08,
remain recorded below.
The failed setup and scheduling attempts remain in raw evidence and do not count
as passes. Do not call the minor merge-ready until every other host's named reruns
and repository gates are complete.

Initial candidate preparation passed at `478d39de3b53989cee2f4894f65bc1427698927a`:
...
helper. The later stale-target repair is recorded below with its own source,
wheel and bundle identities. Automated results alone did not close a live cell;
the WSL verdict above comes from the completed public observations.
...
WSL verified the repaired E01, B09/F03 and B10/B15 observations with wheel
SHA-256 `2f04845b190decee6d3ed5d8e3433812379c42aa451606b06d8604f7282bdb51`.
Their earlier captures are filed through evidence commit `008c48e`.
Eleven unaffected observations retain the preceding candidate's identity and
raw streams. The failed original first-stale requests remain recorded; the
passing repair does not erase them. The final WSL boundary closes E03, E04,
B02f and F08 with hash-bound public streams and independent validation.

The native Windows pass found that the public `trusted` browser-click flag was
documented but omitted from the asynchronous browser signature and dispatch
parameters. Commit `2d0eed9a47cf359ce360c694d1b4b66a2e5e6582` forwards it and adds a
regression test. The repaired wheel SHA-256 is
`d2ed55531e0aad537d16f2523c0a0da50bbc98b88c185155ee990288144285fa`.
The bundle and extension bytes are unchanged. B07 and F05 were rerun after the
repair; the failed original call remains in raw evidence.
...
### Latest extension repair and host audits

The macOS audit used `c2189e959661520d4427ce7e7bdc1db39fbfc652`, wheel SHA-256
`60d5b888ed2f25d24fe3310fa8024f61141cce8240b41e0265f85081770a542a`.
Two inactive trusted clicks returned success with counter zero; active trusted
and inactive DOM controls each incremented their counter. Extension repair
`d7b6aa99f9c76ed2390020053f03cb4ef3e88c9e` is now verified live on macOS and
WSL. Native Windows completed the same affected reruns at `785df68` and
native Linux completed them at `2679395`.

This amendment supersedes the preceding retention map: E01, B07, C06 and F05
must rerun on all four hosts with the new wheel and matching extension. Also
reopen any state-bearing click observation that used automatic DOM-to-CDP
fallback. Plain DOM C01, covered-refusal C02 and DOM-only C05 did not use that
path. Typing, recovery, pixel input and trusted keyboard refusal are unchanged;
C03-C04, C07, Part D and E02 remain at their original artifact identities.
The Windows broker payload is unchanged. Tests do not replace live reruns.
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| B01 | One profile; two independent MCP clients | Attach both clients and list profiles, windows, and tabs | Both remain connected through one broker and see one complete registry | Both JSON streams and broker identity | pass: two independent clients retained one complete registry; unaffected observation retained | pass: two fresh installed clients shared one broker identity and complete registry at d794ad1 | pass: two independent installed clients retained one complete registry and broker identity | pass: distinct installed clients shared broker 37070, epoch 4520d4960d1ebd91 and complete registry at c2189e9 |
...
| B02d | Mirrored host; WSL client starts before native Windows | Repeat B02a | Same B02a identity and routing oracle; WSL uses the stdio proxy, not shared localhost | Both streams, Windows listener and broker identity | pass: WSL-first and native Windows clients shared one broker with exact interleaved reads | Not-Needed: no cross-OS seam | pass: WSL-first then native startup shared one broker with distinct clients and exact independent reads | Not-Needed: no cross-OS seam |
| B02e | Mirrored host; native Windows client starts before WSL | Repeat B02b | Same B02b identity and routing oracle; WSL uses the stdio proxy | Both streams, Windows listener and broker identity | pass: native-first readiness preceded WSL startup; both clients retained exact separate fixture reads | Not-Needed: no cross-OS seam | pass: native-first then WSL startup shared one broker with distinct clients and exact independent reads | Not-Needed: no cross-OS seam |
| B02f | Mirrored host; neither client started | Repeat B02c | Same B02c winner and routing oracle; WSL uses the stdio proxy | Both streams, winner identity and listener | pass: independent native and WSL readiness requests overlapped and converged on one broker; separate fixture reads and listener identity verified | Not-Needed: no cross-OS seam | pass: parallel native and WSL readiness calls elected one broker and exact separate reads | Not-Needed: no cross-OS seam |
...
| B05 | One unowned tab in a shared user window | Client one claims the tab; client two reads it and attempts a window action | Client one can use tab-scoped DOM ops; client two receives `target_owned_by_another_client`; neither client changes window focus | Both streams and page record; release the tab | pass: foreign stale read and parent-window action failed before dispatch; fixture and focus unchanged | pass: foreign tab and parent-window actions failed before dispatch while fixture and focus stayed unchanged at d794ad1 | pass: foreign tab read and parent-window action failed before dispatch; fixture and focus stayed unchanged | pass: foreign stale read and shared-parent action failed without changing document, focus or leases at c2189e9 |
| B06 | Unowned tab and window targets | Race two clients for the tab, then race a window claim against a foreign child lease | One atomic winner exists; every loser gets the named conflict before dispatch | Both streams and lease revision record; release winner | pass: two independent CLI drivers overlapped both claim races; one winner and one named conflict each | pass: bounded MCP barriers queued both claims before either result; each tab and window race had one winner and one named conflict | pass: simultaneous paired claims produced one atomic winner and one named conflict in both races | pass: two independent installed drivers were paused at dispatch; each tab and window race produced one winner and one named conflict at bf92090 |
| B07 | Owned window, child tabs, popup, and shared window | Open tabs with and without opener, a popup, and an unattributable shared-window child | Owned descendants inherit the correct lease; the unattributable shared child stays unowned | Registry snapshots; close created targets | pass: same-window child and real opener-linked popup inherited their lease while a genuinely unattributable browser-created shared child stayed unowned at 2845224/bf92090 | pass: same-window and opener-linked children inherited their leases while a genuinely unattributable browser-created child stayed unowned without a revision bump at 2679395 | pass: opener-linked and owned-window children inherited their leases while the opener-less shared-window child stayed unowned in both clients at 785df68/bf92090 | pass: same-window child and trusted popup inherited their lease while a DevTools-created shared child without an opener stayed unowned at bf92090 |
| B08 | Client owns one tab and one multi-tab window | Release each, then dispatch with the old revision | Browser state stays open, leases clear, and stale revisions fail | Registry and page records; close created state | pass: releases kept resources open and stale revisions were refused | pass: released resources stayed open and old revisions were fenced without mutation at d794ad1 | pass: releases kept resources open and old revisions failed without mutation | pass: released tab and window stayed open and unowned while both stale revisions failed without mutation at bf92090 |
| B09 | Client owns targets; second client stays connected | Break the first socket, reconnect inside 30 seconds, then repeat after heartbeat and grace expiry | Timely reconnect restores state; expired leases become orphaned and are explicitly reclaimable | Streams and monotonic timing; release reclaimed state | pass: repaired candidate restored the 8.032-second cut; 46.063-second cut orphaned leases before explicit claims | pass: the 8.095-second cut opened a real relay connection 11.298 seconds after the cut and retained its lease; the 46.075-second controller completed within its strict bound, orphaned the expired target and explicitly reclaimed it at 2679395 | pass: 8.031-second cut retained the lease; 46.031-second cut expired it for explicit reclaim | pass: an 8.081-second cut reconnected in 8.112 seconds and retained the lease; the 46.057-second cut expired it for explicit reclaim at bf92090 |
| B10 | Browser resources remain open | Restart only the broker | New epoch marks rediscovered targets orphaned; old secrets and revisions fail; URL and title do not restore identity | Before and after registry plus epoch; reclaim or close test state | pass: both first stale requests returned target_lost before status; explicit reclaim preserved documents | pass: PID 122294/epoch 25fde4b9 changed to PID 145054/epoch 89d79d14; first stale request returned target_lost and explicit reclaim preserved the document | pass: replacement broker changed identity, first stale calls returned target_lost, and documents survived reclaim | pass: PID 65846/epoch 8fb0dad633bb844d changed to PID 68170/epoch 9208d49448d8937b; both first stale calls returned target_lost and explicit reclaim preserved both documents at bf92090 |
...
| B15 | Broker running with owned resources | Terminate only that broker process and reconnect both clients | One new Windows PID and epoch appear; rediscovered leases are orphaned | Before/after identity and registry | pass: repaired bundle replacement changed PID and epoch; both registries showed surviving orphaned resources | Not-Needed: no cross-OS seam | pass: replacement changed PID and epoch and rediscovered resources were orphaned | Not-Needed: no cross-OS seam |
...
| C06 | C05 state plus state-bearing pointer, reveal, focus, file-input, dialog and semantic controls | Run trusted `click`, `hover`, `focus`, `blur`, `upload`, `snapshot`, `accessibility_tree`, `eval`, navigation, reload, back and forward; arm the dialog on the leased tab while a decoy target also emits one | Every operation reaches the leased tab, carries an independent state/read-back oracle, ignores the decoy dialog and preserves false/false/true; cookie operations are excluded because they are profile/origin scoped | Before/after target state, state-bearing results, temporary upload hash and decoy hash; remove the temporary file and close only the owned window | pass: active and unfocused inactive trusted clicks delivered isTrusted events; state, upload, semantic, navigation and isolated-dialog oracles passed without activation rescue at 2845224/bf92090 | pass: active and inactive trusted clicks delivered genuine trusted events; all state, upload, semantic, navigation and leased-dialog oracles passed while the inactive target stayed false/false/true and the decoy stayed isolated at 2679395 | pass: active and unfocused inactive trusted clicks delivered isTrusted events; state, upload, semantic, navigation and isolated-dialog oracles preserved false/false/true without activation rescue at 785df68/bf92090 | pass: active and inactive trusted clicks delivered isTrusted events; all state, upload, semantic, navigation and leased-dialog oracles passed with the decoy isolated at bf92090 |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E01 | Fresh environment outside checkout | Install only the built wheel and start `vadgr-cua` through its entry point | Version is 0.7.6, readiness succeeds, the Unicode segmenter and fitted profile load, and source checkout is absent from import paths | Install log, path, version, wheel hash; remove environment | pass: fresh noneditable wheel 788ef13, matching native extension and installed entry point passed readiness, Unicode and fitted-profile checks outside the checkout at 2845224/bf92090 | pass: fresh noneditable wheel 6c9a4fb, matching development extension and installed entry point passed readiness, Unicode and fitted-profile checks outside the checkout at 2679395 | pass: fresh noneditable wheel ff0bfef, matching development extension and installed entry point passed readiness, Unicode and fitted-profile checks outside the checkout at 785df68/bf92090 | pass: fresh noneditable wheel f19d2fe, matching development extension fe80121c and installed entry point b75bffcc passed readiness, Unicode and fitted-profile checks outside the checkout at bf92090 |
...
| E03 | Fresh installed wheel; spaced Windows deployment parent; no test broker running | Request public readiness, open the fixture and read it without manually launching the broker | Normal startup creates one verified broker and the public read succeeds | Path-shape booleans, wheel and bundle hashes, broker identity and MCP results; restore registrations and stop only test processes | pass: WSL-first public startup created the spaced deployment and one verified broker; awaited fixture reads, hashes and restored registrations verified | Not-Needed: native Linux does not execute the Windows launcher | pass: automatic spaced-path startup created one verified broker and exact fixture read | Not-Needed: native macOS does not execute the Windows launcher |
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| E04 | Fresh test-owned Windows deployment absent; no test broker; two independent installed clients | Start both clients concurrently, read an owned fixture from each, then connect a third fresh client | One verified broker; manifest comparison finds no nested staging directory or extra payload; all three clients connect | Process starts, public reads, complete broker identities and file hashes; stop only test processes and remove only isolated state | pass: overlapping clients produced separate exact reads, a third client converged, and 27/27 manifest entries matched with no extra or staging payload | Not-Needed: native Linux does not execute the Windows installer | pass: parallel native and WSL clients plus a third shared one broker; 27/27 files matched with no extra or staging payload | Not-Needed: native macOS does not execute the Windows installer |

## Part F: final audit regressions

The F cells are additional audit cases with current results below. Pure-logic
audit reproductions are not live failures and cannot fill these slots. Use the candidate gate and existing
...
| id | precondition and setup | action or goal | expected observable and oracle | evidence and cleanup | WSL | Linux | Windows | macOS |
|---|---|---|---|---|---|---|---|---|
| F01 | Two isolated profiles connected to one broker; independent clients A and B; one instrumented target per profile | Each client selects a different profile, interleaves status and profile lists, then reads and mutates its own target; switch A to B's profile and repeat status/list without altering B's selection | Each reply marks exactly the caller's selected profile current and agrees with its list; no false profile_ambiguous after selection; original per-profile mutations read back exactly; switching A does not change B | Both public streams, profile/target IDs, exact hashes or counters and unchanged peer records; release targets and close both test profiles | pass: independent profile selections and caller-specific replies matched exact fixture reads | pass: two isolated profiles kept caller-specific current flags and exact independent read-backs across A's profile switch | pass: independent profile selections and caller-specific replies preserved exact fixture reads | pass: two-profile caller-specific selection and exact peer read-backs retained at c2189e9 |
| F02 | Two isolated profiles connected; A explicitly selects one; B is a fresh client with no selection | B requests status and profile list before any target call; A reads its selected target; B explicitly selects the other profile and repeats status/list and a target read | B initially has no current profile and reports profile_ambiguous without inheriting A's selection or creating a target; explicit selection resolves ambiguity only for B; A retains its original selection and document | Both streams, before/after registry and fixture records; close only test resources | pass: fresh unselected client received profile_ambiguous without changing selected peers | pass: the fresh third client stayed unselected and profile_ambiguous until explicit use; A retained its exact profile and document | pass: a fresh client stayed unselected and profile_ambiguous until its explicit selection | pass: fresh unselected client stayed ambiguous until explicit selection; original peer unchanged at c2189e9 |
| F03 | A owns a window with two fixture tabs and selects the second tab; B holds a separate live target; disconnect A and verify heartbeat plus grace expiry without restarting the broker | B explicitly claims only the first orphaned tab, reads and mutates it, then attempts a parent-window close; inspect the unclaimed sibling; reconnect A and attempt its stale sibling write before status or selection; explicitly claim the sibling last | First tab is usable by B with a new tab lease; sibling stays orphaned until its own claim; parent-window close and A's first stale write fail with target_owned_by_another_client; A retains its client identity in the same epoch; no refused action mutates or closes a target; all documents and B's original peer target survive | Both streams, timed expiry, lease scopes/revisions, named errors, document counters and sibling hashes before/after; explicitly reclaim for cleanup and close only test windows | pass: expired sibling stayed orphaned; first stale write and parent close failed before dispatch | pass: after expiry only the explicitly claimed tab moved; sibling, peer documents and refused parent/stale actions remained unchanged | pass: after the 46.031-second expiry only the claimed tab moved; the sibling stayed orphaned and both refused actions were non-mutating | pass: after the 46.057-second expiry B claimed only the first orphan; parent close and A's first stale sibling write failed before dispatch, and all three documents survived at bf92090 |
| F04 | A owns only an opener tab in a shared test window; real opener-linked child tab inherits A's lease; A selects that child; another unrelated tab stays unowned | A releases the inherited child without changing its selection; both clients list twice; B explicitly claims the child, reads and mutates it; A attempts a stale child write before any new selection; inspect opener afterward | Lists never restore A's released child lease; B's claim is usable; A's stale write fails with target_owned_by_another_client; opener remains A's, unrelated tab stays unowned, and all targets remain open without focus change | Both streams, opener and lease IDs, repeated registries, exact read-back and unchanged opener/unrelated-tab records; release and close test targets | pass: released inherited child stayed unowned until explicit claim; stale write refused and peers unchanged | pass: browser-level attach preserved selection; repeated lists did not restore the released child and A's stale write was refused without peer changes | pass: released inherited child stayed unowned until explicit claim; stale write failed and peers were unchanged | pass: repeated lists did not restore the released child; B's claim worked, A's stale write failed, and opener/unrelated read-backs stayed unchanged at bf92090 |
| F05 | A owns an opener target; a real opener-linked popup in another window inherits A's window lease; A selects the popup tab; B is independent | A releases the popup window without changing its selection; both clients list twice; B claims the popup window and uses its existing tab; A attempts a stale popup write before any new selection; inspect opener afterward | Popup stays open and unowned after release and repeated lists; B receives a usable lease; A's stale write fails with target_owned_by_another_client without closing or mutating it; A retains only its opener | Both streams, opener relationship, window/tab leases and revisions, popup read-back and unchanged opener; release and close only test windows | pass: repeated lists left the released popup unowned; B claimed and mutated it once, A's stale write failed, and popup/opener/focus read-backs remained exact at 2845224/bf92090 | pass: repeated lists left the released popup open and unowned; B claimed and used it, A's stale write failed, and the opener remained unchanged at 2679395 | pass: repeated lists left the released popup unowned; B claimed and mutated it once, A's stale write failed, and popup/opener/focus read-backs remained exact at 785df68/bf92090 | pass: repeated lists left the released popup unowned; B claimed it, A's stale write failed, and popup/opener read-backs remained exact at bf92090 |
| F06 | POSIX only; isolated endpoint and native-host discovery absent, ordinary umask, fresh installed wheel and no test broker; metadata observer prepared before startup | Agent requests public readiness and performs an owned fixture read through normal automatic startup | One broker starts; both endpoint and discovery parents are owner-only; endpoint, native-host discovery and their temporary files are owner-readable/writable only; no insecure or leftover publication file; readiness and exact read succeed; deterministic first-write protection proof accompanies this live boundary | Public stream, metadata-only permission observations, installed hashes and broker identity; stop test processes and remove only the isolated root | Not-Needed: WSL uses the Windows endpoint; F08 covers its ACL | pass: fresh wheel startup published 0700 parents and 0600 files, exact fixture read succeeded, and deterministic publication helpers passed 8 tests | Not-Needed: Windows ACL publication is F08 | pass: protected publication, broker identity and original fixture reads at c2189e9; deterministic first-write proof retained |
| F07 | POSIX only; F06 installed candidate; forcibly terminate only the recorded disposable broker PID with a bounded wait; verify its exit and the still-protected stale endpoint before arming the publication-specific fault fixture | Request readiness with the publication fault active; verify refusal, restore the fixture, then request readiness and explicitly reclaim/read the original test document | Failed startup never reports ready or publishes a readable token; existing protected endpoint is not replaced with insecure state; no usable failed broker or temporary token remains; restored startup creates one protected replacement and preserves the original browser document | Public failure/success streams, exit codes, metadata-only before/fault/after records and new broker identity; disarm the child-only fault in bounded cleanup and stop test processes | Not-Needed: WSL uses Windows ACLs; F08 and candidate fault regressions apply | pass: publication_refused preserved the 0600 stale endpoint; restored PID 153387/epoch 137bc9c1 replaced PID 151451/epoch fc80aae1 and preserved the exact document | Not-Needed: Windows ACL publication is F08 | pass: publication_refused preserved the protected stale endpoint and left no failed publisher or temporary; after bounded expiry and disarm PID 69299/epoch 20b47552ccc2c888 replaced PID 68170 and the exact fixture time origin was reclaimed and read at bf92090 |
| F08 | Windows endpoint and native-host discovery absent in test-owned root; fresh verified payload; metadata observer before startup; WSL must inspect Windows ACLs natively | Start via the installed public entry point, read a fixture, then replace only the test broker and explicitly reclaim/read the same target | Endpoint, native-host discovery and temporary files restrict access to the owner and SYSTEM; replacement retains current-token ownership and protected, non-inherited ACLs; one verified broker per generation and original document read-back; caller environment and owner registrations are restored; attach the separate deterministic ACL-failure proof without calling it a live observation | Public streams, current-token owner match and protected, non-inherited DACL trustee/rights metadata without token content, bundle/installed hashes, before/after broker identity and registration comparison; restore only temporary setup and stop test processes | pass: both observed generations retained protected non-inherited owner/SYSTEM ACLs; WSL-first replacement preserved and reclaimed the exact fixture document | Not-Needed: POSIX publication is F06-F07 | pass: both broker generations retained protected owner/SYSTEM ACLs; stale identity failed and the exact document survived reclaim | Not-Needed: POSIX publication is F06-F07 |

## Per-OS results

**Current verdict: WSL, Linux, Windows and macOS complete.**
WSL completed E01, B07, C06 and F05 at `2845224`/`bf92090`. Native Windows
completed the same reopened cells at `785df68`/`bf92090`. Those cells remain
Native Linux completed the four reopened cells and a strict B09 rerun at
`2679395`. The macOS audit completed every reopened or affected cell
at exact candidate `bf92090cea08522a0505d2b2ef4897a61c59b22c`. The owner
explicitly approved GPT-5.6 Luna high after the selected Claude driver refused
some live fault tasks; the test prompts and oracles were not weakened. Unaffected
macOS observations retain their original artifact identities and raw oracles.

| part | WSL | Linux | Windows | macOS |
|---|---|---|---|---|
| A: recovery and setup diagnosis | pass: unaffected observations retained | pass: unaffected observations retained | pass: unaffected observations retained | pass: A03 and A04 reran at bf92090; unaffected observations retained |
| B: shared broker and target ownership | pass: B07 reran at 2845224/bf92090; other accepted results retained | pass: B07 and strict B09 reran at 2679395; other accepted results retained | pass: B07 reran at 785df68/bf92090; other accepted results retained | pass: B06-B10 reran at bf92090; unaffected observations retained |
| C: actionability and browser typing | pass: C06 reran at 2845224/bf92090; unaffected cells retained | pass: C06 reran at 2679395; unaffected cells retained | pass: C06 reran at 785df68/bf92090; unaffected cells retained | pass: C06 reran at bf92090; unaffected observations retained |
| D: pixel typing | pass: unaffected observations retained | pass: unaffected observations retained | pass: unaffected observations retained | pass: unaffected observations retained |
| E: packaged and clean delivery | pass: E01 fresh installed-artifact verification at 2845224/bf92090 | pass: E01 fresh installed-artifact verification at 2679395 | pass: E01 fresh installed-artifact verification at 785df68/bf92090 | pass: fresh E01 installed-artifact verification at bf92090; unaffected observations retained |
| F: final audit regressions | pass: F05 reran at 2845224/bf92090; other accepted results retained | pass: F05 reran at 2679395; other accepted results retained | pass: F05 reran at 785df68/bf92090; other accepted results retained | pass: F03-F05 and F07 reran at bf92090; unaffected observations retained |
| overall | pass: all 42 applicable cells have accepted observations; 5 cells retain their Not-Needed reasons | pass: all 37 applicable cells have accepted observations; 18 cells retain their Not-Needed reasons | pass: all 41 applicable cells have accepted observations; 6 cells retain their Not-Needed reasons | pass: all 32 applicable cells have accepted observations; 15 cells are Not-Needed with retained reasons |

The final native Linux audit used branch head
`26793958bc831361ee6c13e2e2fed575215a2ee5` and fresh noneditable wheel
SHA-256 `6c9a4fb6973e9be7a583d0353fdbac7d10e50840b2cd5d8f404b50978d1915de`.
The 8-second B09 cut restored after 8.095 seconds and opened a real relay
connection 11.298 seconds after the cut, retaining the lease. The 46-second
fault completed after 46.075 seconds within the strict 55-second controller
bound; the expired target became orphaned and was explicitly reclaimed. The
broker process and epoch remained unchanged. The earlier scheduling-blocked
Linux attempt remains historical raw evidence and does not override this pass.
...
At c2189e9, inactive trusted clicks twice reported success without delivering
the independent click event. Repair d7b6aa9 was rebuilt and rerun at bf92090:
active and background trusted clicks both delivered `isTrusted:true` events,
and the complete C06 state/read-back suite passed with the decoy isolated.

The earlier F07 restoration provider refusal remains in raw history and did not
receive a pass. After the owner explicitly approved the qualified Codex model,
the bf92090 rerun observed `publication_refused`, a mode-0600 temporary, the
unchanged protected stale destination during refusal, no remaining failed
publisher or temporary, and a protected replacement after bounded expiry plus
explicit disarm. Public status, reclaim and read-back preserved the exact
fixture time origin across broker PID 68170 to 69299 and epoch
9208d49448d8937b to 20b47552ccc2c888.

The native-host shutdown fatal was reproduced deterministically and repaired at
19c79ad by stopping and joining the input thread before interpreter shutdown.
The regression went red before the repair and green afterward. The final bf92090
broker-client preflight repair likewise retains no-replay semantics and received
fresh B10, A03, A04, B09, F03 and F07 live coverage. No unresolved macOS finding
remains at this candidate boundary.

The first clean-install smoke omitted the isolated child home and changed
normal native-host registration state. That smoke is rejected. The owner
approved restoration of the observed prior launcher target; existing owner
processes remained unchanged. Subsequent installed startup used isolated child
state. One Chrome startup still used the normal Crashpad location; the next
startup supplied an isolated crash directory. No owner crash data was inspected
or removed. Current cleanup is recorded separately and does not inherit earlier
completed cleanup claims.
